...
- 生产运行使用本地配置的 Flash/Pro Provider；模型和 Key 通过 `LEARNINGLOOP_*` 注入，日志只记录 provider alias。
- 备用 Provider 默认不进入路由；设置 `LEARNINGLOOP_ENABLE_RELAY_FALLBACK=true` 后才允许显式故障转移。
