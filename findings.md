...
- Runtime provider settings are supplied through the ignored local environment or a deployment secret store.
- Flash and Pro routes may use separate local credentials and model IDs.
...
- Web search uses a separate provider capability check without replacing the Harness adapter.
- Missing usage and price values remain null. Local price data is stored only as a versioned estimate.
...
- Live provider compatibility checks are opt-in and environment-specific; public CI does not claim external provider availability.
...
- Offline scenario P50/P95 includes Python setup, SQLite and service initialization for each isolated case. It must never be presented as live provider response latency.
...
- 使用本地配置的 Provider 路由完成 10 个固定任务和 3 组上下文对照，共 16 次小规模调用。
...
- 同一批固定输入按本地价格表重算：混合路由 `$0.00369006`，反事实 Pro-only `$0.00551298`，估算降低 33.07%。
